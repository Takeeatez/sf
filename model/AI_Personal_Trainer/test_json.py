...
# 팔꿈치 위치 추적을 위한 변수
elbow_positions = {'right': deque(maxlen=30), 'left': deque(maxlen=30)}  # 1초(30fps * 1초)
...
def check_elbow_stability(side):
    if len(elbow_positions[side]) < 30:  # 1초 동안의 데이터가 없으면 안정적이지 않음
        return False
    
    positions = np.array(list(elbow_positions[side]))
    mean_pos = np.mean(positions, axis=0)
    distances = np.sqrt(np.sum((positions - mean_pos)**2, axis=1))
    return np.mean(distances) < 50  # 평균 거리가 50픽셀 이내면 안정적으로 간주 (미세한 떨림 허용)
            ...
            if exercise_type == "아령 들기":
                ...
                if not elbow_fixed:
                    ...
                    if right_stable and left_stable:
                        ...
                        if stable_duration['right'] >= 30 and stable_duration['left'] >= 30:  # 1초 동안 안정적
                ...
                # 안내 메시지
                if not elbow_fixed:
                    img = put_text(img, "팔꿈치를 고정 위치에 1초간 유지하세요", (50, 50))
                    img = put_text(img, f"안정화: {min(stable_duration['right'], stable_duration['left'])/30:.1f}/1.0초", (50, 100))
                else:
                    # 운동 중 로직 (각도 계산, 카운트 등)
